...
/**
 * 长按弹窗，复制，粘贴，转发，，，，，，
 *说明，此组件需要配合重写activity的dispatchTouchEvent方法获得用户长按或者点击的坐标
 * @author admin
 */
public class HorizontalMenuPopuWindow extends PopupWindow {
    ...
    private static HorizontalMenuPopuWindow popupList = null;
    ...
    public static synchronized HorizontalMenuPopuWindow getInstance(Context context) {
        if (popupList == null) {
            popupList = new HorizontalMenuPopuWindow(context);
        }
        return popupList;
    }
        ...
        private void addPopuListItem(LinearLayout popupListContainer) {
            for (int i = 0; i < config.mPopupItemList.size(); i++) {
                ...
                if (config.mPopupItemList.size() > 1 && i == 0) {
                    ...
                    setBackground(textView,mRightItemBackground);
                } else if (config.mPopupItemList.size() == 1) {
                    setBackground(textView,mCornerItemBackground);
                } else {
                    setBackground(textView,getCenterItemBackground(config));
                }
    ...
    private LinearLayout createContainerView() {
        ...
        setBackground(popupListContainer,mCornerBackground);
